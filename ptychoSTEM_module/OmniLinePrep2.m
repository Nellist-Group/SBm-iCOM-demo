function omniline = OmniLinePrep2(kx,ky,qx,qy,nth)
...
% - { chi(Kf+Qp) - chi(Kf)  - phase(Psi_s(-Qp)) }
% ,where phase(Psi_s(-Qp)) is a constant

% the phase of A(Kf-Qp)A.*(Kf)Psi_s(Qp)equals to:
% chi(Kf-Qp) - chi(Kf) + phase(Psi_s(Qp))
% ,where phase(Psi_s(Qp)) is a constant
