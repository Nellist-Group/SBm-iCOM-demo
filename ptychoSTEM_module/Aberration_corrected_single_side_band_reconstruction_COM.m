function [ptycho] =  Aberration_corrected_single_side_band_reconstruction_COM2(ptycho)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
...
%         trotterphasesumRC(yy,xx) = sum(trotterRC(:));
        ...
        ssb_mask_G_Qf = g_ampL.*exp(1j.*g_phaseL).*(func_transfer.*conj(func_transfer_plusQ))...
            + g_ampR.*exp(1j.*g_phaseR).*(func_transfer_minusQ.*conj(func_transfer));
        COMxf(yy,xx) = sum(sum(-dx.*(ssb_mask_G_Qf)));
        COMyf(yy,xx) = sum(sum(-dy.*(ssb_mask_G_Qf)));
...
Qpy = repmat(ptycho.Q_y',1,ptycho.ObjSize(2));  % dim 4
Qpx = repmat(ptycho.Q_x,ptycho.ObjSize(1),1);  % dim 3

Qp2 = Qpx.^2 + Qpy.^2;

Qp2(Qp2 == 0) = 1e-8;

COMxfr = -(COMxf * cos(ptycho.rot_angle) + COMyf * sin(ptycho.rot_angle));
COMyfr = -(-COMxf * sin(ptycho.rot_angle) + COMyf * cos(ptycho.rot_angle));

fKr = (Qpx .* COMxfr + Qpy .* COMyfr)./(2 .* pi .* 1j .* Qp2);
% fK = (Qpx .* COMxf + Qpy .* COMyf) ./ (2 .* pi .* 1j .* Qp2);


iCOM = real(fftshift(ifft2(ifftshift(fKr))));

COMx = real(fftshift(ifft2(ifftshift(COMxf))));
COMy = real(fftshift(ifft2(ifftshift(COMyf))));

COMxr = real(fftshift(ifft2(ifftshift(COMxfr))));
COMyr = real(fftshift(ifft2(ifftshift(COMyfr))));

[gxx,gxy] = gradient(COMxr);
[gyx,gyy] = gradient(COMyr);
dCOM = -(gxx + gyy);
