...
COMx = zeros(dpNum(1),dpNum(2),length(rotate_angle));
COMy = zeros(dpNum(1),dpNum(2),length(rotate_angle));
...
%         COMx(ii,jj,:) = directional_centroid(dps_4d(:,:,ii,jj),-rotate_angle_rad);
%         COMy(ii,jj,:) = directional_centroid(dps_4d(:,:,ii,jj),-rotate_angle_rad-pi/2);
    end
end
...
%     IcomyT = IcomyT - mean(IcomyT(:));
    ...
    Result(kk).COMx = COMx;
    Result(kk).COMy = COMy;
    Result(kk).COMxr = IcomxT;
    Result(kk).COMyr = IcomyT;
    ...
    COMxf = Simu4dstem_fft2(Result(kk).COMx);
    COMxf = abs(COMxf) .* fm .* exp(1j .* angle(COMxf));
    Result(kk).COMxf = real(Simu4dstem_ifft2(COMxf));
    
    COMyf = Simu4dstem_fft2(Result(kk).COMy);
    COMyf = abs(COMyf) .* fm .* exp(1j .* angle(COMyf));
    Result(kk).COMyf = real(Simu4dstem_ifft2(COMyf));
...
function c = directional_centroid(arr, angle)
...
% angle      : direction angle (radians)
    ...
    weight = zeros(size(arr,1),size(arr,2),length(ux));
    c = zeros(1,length(ux));
    for ii=1:length(ux)
        weight(:,:,ii) = X .* ux(ii) + Y .* uy(ii);
        c(ii) = sum(arr .* weight(:,:,ii), 'all');
    end
