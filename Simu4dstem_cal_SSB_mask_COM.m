function [ptycho_0,Res] = Simu4dstem_cal_SSB_mask_COM(jsonData,dps_4d)
...
% parameters change
ptycho.identifier = 'Simu4dstem_cal_SSB';
...
if isfield(jsonData,'SSB_super_sampling_step_size') && jsonData.SSB_super_sampling_step_size ~= 0
    ptycho.super_sampling_step_size = jsonData.SSB_super_sampling_step_size;
end
...
% if want aberration correction SSB, use following testing code.
% [ptycho] = define_probe_function_Modified2(ptycho);
% [ptycho] = single_side_band_reconstruction2(ptycho);
% SSB super sampling
[ptycho] = single_side_band_reconstruction_ss_COM(ptycho);
...
fprintf('%s[LOG]: SSB mask COM finished\n',datestr(now,31));
